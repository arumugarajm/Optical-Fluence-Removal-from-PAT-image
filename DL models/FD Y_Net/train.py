...
@author: arumugaraj
"""
from FDYnet import FDY_net,avg_NSR, avg_log_SNR
...
model = FDY_net((256, 256, 1),32,(256,2048,1))
############ set up checkpoint ####################
...
print('average PSNRLoss of FD Y-Net on the testing set: {}'.format(tf.math.reduce_mean(tf.image.psnr(Z3, y_pred, 1.)).numpy()))
print('variance PSNR of FD Y-Net on the testing set: {}'.format(tf.math.reduce_variance(tf.image.psnr(Z3, y_pred, 1.)).numpy()))
print('average SSIM of FD Y-Net on the testing set: {}'.format(tf.math.reduce_mean(tf.image.ssim(Z3, y_pred, 1.)).numpy()))
print('variance SSIM of FD Y-Net on the testing set: {}'.format(tf.math.reduce_variance(tf.image.ssim(Z3, y_pred, 1.)).numpy()))
