...
def read_data(path):
    i1=12   # Number of training dataset
    i2=16   # Number of validation dataset Ex.(16-12=4) 4 dataset used for validation, remaining datset used for testing
    ...
    return np.expand_dims(input_images[: i1], axis=-1).astype('float32'), np.expand_dims(GT_images[: i1], axis=-1).astype('float32'), np.expand_dims(input_images[i1:i2], axis=-1).astype('float32'), np.expand_dims(GT_images[i1:i2], axis=-1).astype('float32'), np.expand_dims(input_images[i2:], axis=-1).astype('float32'), np.expand_dims(GT_images[i2:], axis=-1).astype('float32')
