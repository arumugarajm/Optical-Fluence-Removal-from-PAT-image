myFolder = '/media/fistlab/DATA/Raj/raw/raww/a';
if ~isdir(myFolder)
...
%     fprintf(1, 'Now reading %s\n', fullFileName);
    ...
    r = imgaussfilt(r, 0.9);
    Y1 = Y.*r;
    r1 = 0.03 + (0.04-0.03)*rand(256,256);
    X1 = X.*r1;
...
% vertn=[vert u];
% vertn=[Y -X u];
vertn=0.125*0.5*0.5*[X Y u];% generating node points according to NIRFAST
...
coord=0.125*0.5*0.5*[0.5 22.5;0.5 53.5;0.5 77.5;0.5 99.5;0.5 125.9;0.5 150.9;0.5 176.2;0.5 202.6;0.5 225.7;0.5 246.6];%100x100
%coord=[0.5 0.5;0.5 10.4;0.5 21.4;0.5 31.3;0.5 39;0.5 48.6;0.5 56.3;0.5 65.1;0.5 75;0.5 79.4];%80x80
% coord=[0.125 0.4;0.125 5.075;0.125 10.3; 0.125 14.975; 0.125 20.2;0.125 25.125; 0.125 30.05;0.125 35;0.125 39.95;0.125 45.175];%50x50
...
meas.coord=0.125*0.5*0.5*[256.5,22.5;256.5,48.9;256.5,76.4;256.5,100.6;256.5,127;256.5,152;256.5,175.1;256.5,225.7;256.5,246.6];
...
% plotmesh(mesh,0);% inbuilt function used to plot mesh
...
for i=1:row  % checking which point is nearer to the original image index postion
   ...
   Ximg(E2(i),F2(i))=mesh.mua(i);
    ...
    if Ximg(E2(i),F2(i))==0
        Ximg(E2(i),F2(i))= mesh.mua(i);
    elseif Ximg(E1(i),F2(i))==0
        Ximg(E1(i),F2(i))= mesh.mua(i);
        elseif Ximg(E2(i),F1(i))==0
            Ximg(E2(i),F1(i))= mesh.mua(i);
    else 
        Ximg(E1(i),F1(i))=mesh.mua(i);
    end
...
% figure;imshow(Ximg,[]),title('mesh to Image');
% % save('Ximg.mat','Ximg')
% save('C:\Users\arumugaraj\Desktop\X.mat','Xim','Ximg');
...
% object_sim.p01 = zeros(object_sim.Nx, object_sim.Ny);
% object_sim.p01(indxi,indyi) = b(:,:);
% sdnGT = forward(object_sim, time, medium, sensor);



sdn = forward(object_sim, time, medium, sensor);
%imshow(sdn,[]);
% [b, a] = cheby1(10,26,0.9,'low');
% sd2 = filter(b,a,sd21);
...
Xi=IM2_k_interp(123:378,123:378);
save(['/media/fistlab/DATA/Raj/raw/test/AA/X_' num2str(k) '.mat'],'Xim','Xi','sdn','sdn2');
